...
const InactiveCustomer = ({ filteredData, type, currentYearCutomers, currentMonthCustomers, currentWeekCustomers }) => {

  const data = filteredData.filter((item) => item.status === false);
  ...
  }, [currentYearCutomers, filteredData]);
  ...
  function getMonthlyDataCount(dataArray) {
    ...
    dataArray.forEach(data => {
      const createdAt = new Date(data.deactivatedAt);
  ...
  // Filter counts
  const yearly = data.filter((item) => new Date(item.deactivatedAt).getFullYear() === currentYear).length;
  
  const monthly = data.filter((item) => {
    const itemDate = new Date(item.deactivatedAt);
    ...
    <React.Fragment>
      <div className="card-title-group align-start pb-3 g-2">
        <div className="card-title card-title-sm">
          <h6 className="title">{type ? "Customers" : "Inactive Customers"}</h6>
        </div>
      ...
      <div className="analytic-au">
        ...
        <div className=" card-title-sm mt-3">
          <h6 className="title">Total Cutomers : {data?.length}</h6>
        </div>
      </div>
    </React.Fragment>
  );
