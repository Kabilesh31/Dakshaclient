  ...
  if (currentSite) {
    ...
    setPhotos(
      currentSite.additionalImages && currentSite.additionalImages.length > 0
        ? currentSite.additionalImages
        : dummySites[0].additionalImages
    );

    setSitePlanImages(
      currentSite.sitePlanImages && currentSite.sitePlanImages.length > 0
        ? currentSite.sitePlanImages
        : dummySites[0].sitePlanImages
    );

    setProgressHistory(
      currentSite.progressHistory && currentSite.progressHistory.length > 0
    ...
    const getProgressStatusBadge = (status) => {
        switch(status) {
        case "completed": return <Badge color="success" pill>Completed</Badge>;
        case "in-progress": return <Badge color="warning" pill>In Progress</Badge>;
        default: return <Badge color="secondary" pill>Pending</Badge>;
        }
    };
    ...
    return (
        ...
        <BlockHead size="sm">
            ...
            <Button color="dark"
    size="sm"
    className="mb-3 ml-2"  style={{ marginTop: "5.5rem" }} onClick={() => history.push("/SiteManagement")}>
                <Icon name="arrow-left" /> Back
            </Button>
            <BlockTitle page className="ml-2" tag="h3">
                {site.name}
            </BlockTitle>
            <BlockDes className="text-soft ml-2">
        ...
        <Block className="card card-bordered">
            ...
            <Row className="g-4">
                ...
                <div className="d-flex justify-content-between align-items-start">
                    ...
                    <Button color="primary" outline size="sm" className="ms-2" onClick={() => setEditModal(true)}>
                        <Icon name="edit" /> Edit
                    </Button>
                    </div>
                </div>
  ...
  <div className="d-flex justify-content-between align-items-center">
    ...
    <Button color="primary" className="mb-2" outline onClick={() => setPhotoModal(true)}>
      ...
      <Col md="4" key={idx}>
        ...
        <Modal isOpen={photoModal} toggle={() => setPhotoModal(false)}>
            ...
            <div className="d-flex justify-content-end gap-2 mt-3">
                ...
                <Button color="primary" onClick={handleAddPhoto}>
                Add to Gallery
                </Button>
            </div>
            </ModalBody>
        ...
        <Modal isOpen={editModal} toggle={() => { setEditModal(false); setFormErrors({}); }} size="lg">
            <ModalHeader toggle={() => { setEditModal(false); setFormErrors({}); }}>Edit Site Details</ModalHeader>
