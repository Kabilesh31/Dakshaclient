...
const UserProfileNotificationPage = ({ sm, updateSm }) => {
  ...
  const pdfSVG = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72">
   ...
   const pptSVG =  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72">
    ...
    const xlSVG =  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72">
    ...
    const wordSVG = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72">
    ...
    const zipSVG = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72">
  ...
  const handleDropChange = (acceptedFiles) => {
    ...
    setUploadedFile(newFiles[0]); // Assuming you want the latest file
  };
  ...
  const handleUpload = () => {
    ...
    formData.append('file', uploadedFile.file); // Append the file to FormData
    formData.append('fileType', uploadedFile.type); // Optional: Append the file type
  ...
  const downloadFile = (item) => {
    try {
      // Check if item.data is a Base64 string
      const isBase64 = (data) => {
      ...
      if (isBase64(item.data)) {
        ...
        binaryString = atob(item.data);
      } else {
        binaryString = new TextDecoder().decode(item.data); // If it's a Buffer
      }
      ...
      a.download = item.filename; // Set the download attribute to the filename
  ...
  return (
    <React.Fragment>
      ...
      <BlockHead size="lg">
        <BlockBetween>
          <BlockHeadContent>
            ...
            <Button onClick={toggleAssignModal} color="primary" size="md">Upload</Button>
            </div>
            {/* <BlockDes>
      ...
      <BlockHead size="sm">
        <BlockBetween>
          <BlockHeadContent>
            <BlockTitle tag="h6">Files</BlockTitle>
            <BlockDes>
      ...
      {driveData.map((item, idx) => {
            ...
            return (
              <Col lg="3" sm="4" md="6" key={idx}>
                <div  onClick={() => downloadFile(item)}  className="preview-icon-box card card-bordered">
