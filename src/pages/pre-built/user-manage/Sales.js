...
const Sales = () => {
  ...
  const fetchAssignmentsByDate = async (date) => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_BACKENDURL}/api/route-assignment-sales`, {
        params: { date },
        headers: {
          Authorization: `Bearer ${token}`,
          "session-token": sessionToken,
  ...
  const filteredDeliveryStaff = Array.isArray(deliveryStaff)
    ? deliveryStaff.filter((staff) => staff.staffStatus === "active" && staff.isDeleted === false)
    : [];
  ...
  const handleUnassignRoute = async () => {
    ...
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_BACKENDURL}/api/route-assignment-sales/${selectedAssignmentId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "session-token": sessionToken,
        },
  ...
  const renderRouteAssign = () => {
    ...
    const getAvailableVehiclesForStaff = (staffId) => {
      ...
      return vehicles.filter((vehicle) => {
        const vehicleUsed = usedVehicles.find((v) => v.vehicleNo === vehicle.vehicleNumber);

        if (!vehicleUsed) return true;

        if (vehicleUsed.staffId?.toString() === staffId?.toString()) return true;
                  ...
                  {Array.isArray(filteredDeliveryStaff) &&
                    filteredDeliveryStaff.map((staff) => {
                      ...
                      const staffVehicleAssignment = routeAssignments.find(
                        (a) => a.staffId?._id?.toString() === staff._id?.toString() && a.date === selectedDate,
                      );
                        ...
                        <tr key={staff._id} style={{ borderBottom: "1px solid #edf2f7" }}>
                          ...
                          <td style={{ padding: "12px 16px" }}>
                            {staffAssignments?.length > 0 ? (
                              ...
                              <span style={{ color: "#a0b3c9", fontSize: "0.85rem", fontStyle: "italic" }}>
                                No routes assigned
                              </span>
                            )}
                          </td>
                            ...
                            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                              ...
                              <button
                                style={{
                                  ...
                                  transition: "all 0.2s",
                                  boxShadow:
                                    !staff.selectedRoute || loading ? "none" : "0 2px 4px rgba(44,107,158,0.2)",
                                }}
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    // reset only route
                                    setDeliveryStaff((prev) =>
                                      prev.map((s) => (s._id === staff._id ? { ...s, selectedRoute: "" } : s)),
                                    );
                                  } catch (err) {
                  ...
                  {routeAssignments.map((assignment) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="mb-0 small text-primary">Assigned vehicle: {assignment.vehicleNo || "N/A"}</p>
                      </div>
  ...
  const renderLiveTrack = () => {
    ...
    const calculateWarehouseToFirstCustomerTime = () => {
      ...
      const sortedVisits = [...visitedCustomers].sort((a, b) => new Date(a.visitedAt) - new Date(b.visitedAt));
          ...
          <PreviewCard>
            <div className="d-flex justify-content-between align-items-center mb-4">
              <div>
                ...
                <p className="text-soft mb-0">Track Sales in real-time on the map</p>
              </div>
                    ...
                    <div className="staff-list" style={{ maxHeight: "500px", overflowY: "auto" }}>
                            ...
                            {staffDelivery && (
                              <div className="mt-1">
                                <small className="text-muted fs-11">
                                  Route:
                                  <span className="fw-medium text-dark ms-1">{staffDelivery.routeName}</span>
                                </small>
                                {staff.startedAt && (
                      ...
                      {!showAllStaff && staffWithActiveDeliveries.length === 0 && (
                        <div className="text-center py-5">
                          ...
                          <Button color="primary" size="sm" onClick={() => setShowAllStaff(true)}>
                            Show All Staff
                          </Button>
                        </div>
                      )}
                  ...
                  <div className="card-inner">
                    ...
                    {!assinedCustomerDatas || assinedCustomerDatas.length === 0 ? (
                      <div className="text-center py-5">
                        ...
                        <p className="text-muted mb-0">This staff has no routes assigned for today.</p>
                      </div>
                    ) : (
                      <div className="rail-status">
                        ...
                        <div className="rail-row completed">
                          ...
                          <div className="rail-content">
                            ...
                            <small className="text-muted">
                              Trip Started • 0 km
                              {warehouseToFirstCustomer && (
                                <span className="text-primary ms-2">→ 1st in {warehouseToFirstCustomer.formatted}</span>
                              )}
                            </small>
                          </div>
                        </div>
                          ...
                          .map((customer, index) => {
                            ...
                            if (customer.isVisited === true) {
                              ...
                              const hasPreviousPending = assinedCustomerDatas
                                .sort((a, b) => a.lineNo - b.lineNo)
                                .some((c) => c.lineNo < customer.lineNo && c.isVisited === false);
                            ...
                            const isFirstCustomer =
                              index === 0 && rowStatus === "completed" && warehouseToFirstCustomer;
                            ...
                            return (
                              ...
                              >
                                <div className="rail-left">
                                  ...
                                  {index !== assinedCustomerDatas.length - 1 && <div className="rail-line"></div>}
                                </div>
                                ...
                                <div className="rail-content">
                                  ...
                                  <div className="d-flex justify-content-between align-items-center mt-1">
                                    ...
                                    {rowStatus === "completed" && customer.visitedAt && (
                                      <span className="text-muted small">{formatVisitedTime(customer.visitedAt)}</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                        ...
                        <div className={`rail-row ${isRouteCompleted ? "completed" : "upcoming"}`}>
                          ...
                          <div className="rail-content">
                            <div className="d-flex justify-content-between">
                              ...
                              <span className="time">
                                {selectedTrackStaff?.endedAt ? formatVisitedTime(selectedTrackStaff.endedAt) : "--:--"}
                              </span>
                            </div>
                            <small className="text-muted">
                              Trip Completion
                              {totalDuration && (
                                <span className="text-success ms-2">Total: {totalDuration.formatted}</span>
                              )}
                            </small>
                          </div>
                        </div>
                        ...
                        {isRouteCompleted && (
                          <div className="alert alert-success py-2 mb-3">
                            ...
                            {totalDuration && (
                              <span className="d-block mt-1 small">Total time: {totalDuration.formatted}</span>
                            )}
                          </div>
                        )}
                      </div>
  ...
  const renderBrandAssign = () => {
    return <BrandAssign onStaffUpdate={fetchStaff} salesStaff={deliveryStaff} />;
  };
      ...
      <Content>
        ...
        <Block>
          ...
          <div className="tab-content mt-4">
            ...
            <div className={`tab-pane ${activeTab === "brand" ? "active" : ""}`}>{renderBrandAssign()}</div>
          </div>
          ...
          <Modal isOpen={confirmOpen} toggle={() => setConfirmOpen(false)} centered>
            <ModalBody className="text-center">
              ...
              <p className="text-muted">This action will remove the route from the staff for this day.</p>
