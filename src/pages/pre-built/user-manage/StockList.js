...
const StockList = () => {
  ...
  const onEditSubmit = async () => {
    ...
    // Append only the fields that have been changed (non-empty values)
    ...
    // Include the uploaded file if there is one
    if (uploadedFile) {
      formDataToSend.append("file", uploadedFile.file); // Add the image file
    }

    try {
      const response = await axios.put(`${process.env.REACT_APP_BACKENDURL}/api/product/${editId}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
                ...
                <Form className="row gy-4" onSubmit={handleSubmit(onFormSubmit)}>
                  ...
                  <Col size="12">
                    <Dropzone
                      accept=".png, .jpg"
                      multiple={false}
                      onDrop={(acceptedFiles) => handleDropChange2(acceptedFiles, setFiles)}
                    >
