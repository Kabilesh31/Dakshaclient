...
const PayRollFiles = ({ sm, updateSm }) => {
  ...
  const fetchDriveData  = async() => {
    try{
      const response = await axios.get(process.env.REACT_APP_BACKENDURL+"/api/payrollFiles")
  ...
  const filterDriveData = driveData.filter((item) => item.userId === id)
          ...
          {filterDriveData?.length > 0 ? (
            filterDriveData.map((item, idx) => {
              ...
              return (
                ...
                <div style={{ cursor: 'pointer', height:"190px", padding:"10px"}} onClick={() => openFileInModal(item)} className="preview-icon-box card card-bordered">
            ...
            <div style={{ position: "absolute", right:"50%", bottom:"35%",  alignItems: "center", textAlign:"center"}}>
